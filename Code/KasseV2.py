...
for i in range(len(SETTINGS.teamList)):
    # create buttons
    widgets.teambuttons[i] = tk.Radiobutton(frames.teams, text=SETTINGS.teamList[i].strip(), value=i, variable=selectedTeam, indicatoron = tk.FALSE, bg="orange")
    widgets.teambuttons[i].grid(row=i // 3, column=i % 3, sticky=tk.NSEW, padx=0, pady=0)
    # load 'on' image
    img = Image.open("Logo_"+SETTINGS.teamList[i].strip()+"_On.gif")
    ...
    # load 'off' image
    img = Image.open("Logo_"+SETTINGS.teamList[i].strip()+"_Off.gif")
    img = img.resize((SETTINGS.teamimagesize,SETTINGS.teamimagesize), resample=Image.LANCZOS)
...
widgets.playerAddButton = tk.Button(frames.players, text="Add Player", command=add_player)
...
for i in range(len(SETTINGS.items.name)):
    ...
    widgets.itembutton[i] = tk.Button(frames.itemcategories[SETTINGS.items.category[i]], text=buttontext, font=("Courier", 30))
