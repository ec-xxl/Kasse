...
def popupError(s):
    ...
    popupRoot.geometry("%dx%d+%d+%d" % (400, 50, root.winfo_screenwidth()/2 - 200, root.winfo_screenheight()/2 - 25 ))
    popupRoot.focus_set()
    popupRoot.mainloop()
...
widgets.playerButtonAdd = tk.Button(frames.players, text="Spieler hinzufügen", command=add_player)
...
for i in range(len(SETTINGS.items.name)):
    ...
    widgets.itembutton[i].configure(text=buttontext, font=("Courier", 20), image=pixel, compound="c", height=1, width=1)
...
widgets.totalTreeViewItems = ttk.Treeview(frames.total, columns=('Bestellung','Preis','Anzahl','Gesamt'))
# widgets.totalTreeViewItems["columns"]=("Preis", "Anzahl", "Gesamt")
widgets.totalTreeViewItems.heading("Bestellung", text="Bestellung")
widgets.totalTreeViewItems.heading("Preis", text="Preis")
widgets.totalTreeViewItems.heading("Anzahl", text="Anzahl")
widgets.totalTreeViewItems.heading("Gesamt", text="Gesamt")
widgets.totalTreeViewItems["displaycolumns"]=("Bestellung","Preis","Anzahl","Gesamt")
widgets.totalTreeViewItems["show"]=("headings")
widgets.totalTreeViewItems.column('Bestellung', width=100, stretch=1)
